...
A reverse TCP payload and listener both written in python3. Currently under heavy development.
...
```
The current supported encryptions are hex, base64, and  <a href="https://github.com/MrSharkSpamBot/CadaverousCipher">CadaverousCipher</a>. Before using CadaverousCipher encryption, run GenerateDictionaryKey.py in the ConfigurationTools directory. To send data with newline characters, use the command multiline.
...
### Compile.py
This compilation script can only be run on a Windows computer or Wine environment. To configure the compilation script, go to line 14 and replace ICON with the icon file you want your exe file to have to have. To do this make sure that the payload, icon file, and Compile.py script are in the same folder and that py2exe is installed.
```
